...
class TwoLayerNet(_baseNetwork):
    ...
    def forward(self, X, y, mode='train'):
        ...
        layer1_output = np.dot(X, self.weights['W1']) + self.weights['b1']
        layer1_activation = self.sigmoid(layer1_output)

        layer2_output = np.dot(layer1_activation, self.weights['W2']) + self.weights['b2']
        layer2_activation = self.softmax(layer2_output)

        loss = self.cross_entropy_loss(layer2_activation, y)
        accuracy = self.compute_accuracy(layer2_activation, y)
        ...
        #############################################################################
        out = layer2_activation - y_encode
        normalized_out = out / len(y)
        gradient = np.sum(normalized_out, axis=0)
        self.gradients['b2'] = gradient

        self.gradients['W2'] = np.dot(layer1_activation.T, normalized_out)

        a = np.dot(normalized_out, self.weights['W2'].T)
        b = self.sigmoid_dev(layer1_output)
        self.gradients['b1'] = np.sum(np.multiply(a, b), axis=0)

        c = np.multiply(a, b)
        self.gradients['W1'] = np.dot(X.T, c)
